package com.primeiro.pay.oppwa.payments.preauth.api.rest;

import com.primeiro.pay.oppwa.payments.preauth.domain.entity.Payment;
...
public class PreAuthorizationVertx extends AbstractVerticle {
    ...
    public void start() {
        ...
        //
        final CamelContext preAuthCamelContext = springContext.getBean("preAuthCamelContext", CamelContext.class);
        //
        CamelBridge.create(super.vertx, new CamelBridgeOptions(preAuthCamelContext)
        ...
        router.get(super.config().getString("pathGreet")).handler(handler -> {
            handler.response()
                    ...
                    .end(Json.encodePrettily(new JsonObject()
                            .put("message", "Hi PrimeroPay from PreAuthorization Service")));
        });
        //
